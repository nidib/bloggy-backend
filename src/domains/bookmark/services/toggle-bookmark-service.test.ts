...
describe('ToggleBookmarkSerivce', () => {
	it('Should return true if an article is bookmarked for the first time', async () => {
		const toggleBookmarkSerivce = new ToggleBookmarkService(bookmarkRepository, articleRepository, userRepository);
	...
	it('Should throw exception if user does not exist', async () => {
		...
		await expect(toggleBookmarkSerivce.execute(randomUUID(), randomUUID())).rejects.toMatchObject(
